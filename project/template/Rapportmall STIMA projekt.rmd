---
title: "{En informativ titel ska vara här}"
subtitle: "Kurskod, GruppX"
author: 
  - "Författare 1 Namn"
  - "Författare 2 Namn"
date: '20XX-XX-XX'
...
Ha som riktmärke att rapporten ska vara 10-20 sidor (exkludera referenser och bilaga från den siffran). Det är viktigt att det inte ska vara för långt för opponenter och examinator att läsa. Försök att hålla rapporten under 20 sidor brödtext.
...
Notera: Alla figurer, tabeller och liknande ska vara numrerade och ska refereras från brödtexten. Alltså inga orefererade figurer/tabeller! När ni ska kommentera/analysera figurer/tabeller etc, referera till figuren/tabellens nummer: "i figur 1 ses det att...", undvik formuleringar som "i tabellen nedan/ovan..."
...
## Bakgrund
I bakgrunden ges en introduktion till ämnet och motivering till problemställningen i uppsatsen. Här ska ni ha minst olika **2 källor** som behandlar ämnet/problemet. Sen om ni hittar andra som har analyserat ert dataset tidigare: citera dessa och skriv kortfattat "vad som har gjorts på data". (Om data har använts väldigt mycket så räcker det att citera några användningar.)
...
Undvik onödigt stora plottar/figurer. 

Om man har många variabler som man vill plotta $\rightarrow$ överväg att lägga en del av dessa i Bilagan och skriv  något i stil "att fler plottar finns i bligan..."

Om man har ett antal figurer som är mycket lika varandra $\rightarrow$ visa en i själva rapporten och lägg resten i Bilagan och skriv att resterade plottar finns i Bilagan. Detta gäller alla delar av rapporten. 
...
## Teori
Ni ska **kortfattat** förklara era valda metoder i allmänna termer. Det räcker med ett antal meningar/några stycken per metod. Skriv något kort om hur modellerna skattas/optimeras. Ni behöver inte visa formler om ni inte vill. Fokusera på metodernas/modellerna övergripande egenskaper/koncept. Ha litteraturhänvisningar för era metoder.

Tänk att denna sektion ska vara 1-2 sidor

*Notera*: Vanlig linjär regression och linjär logistisk regression behöver inte förklaras men ska nämnas om det användas. Om man använder någon utökning av linjär (logistisk) regression så ska det förklaras.

I de flesta fall så vill man jämföra med enkel baslinjemetod (baseline), och då brukar oftast linjär (logistisk) regression vara den bästa alternativet för det. Man kan ha en egen rubrik "Baslinjemetod" där man skriver kortfattad om vilken baslinjemetod man använder. Se nedan för mer info om baslinjemetod.

## Praktisk metod

Fokus i metodkapitlet ska vara i denna sektion.
...
- Om ni vill hålla er till endast en metod så bör ni motivera det och den metoden bör utforskas mer ingående med avseende på olika inställningar etc. (Ett exempel kan vara: ni arbetar med bilddata och använder bara olika typer av neurala nätverk)
- **Ha som grund**: minst 2 olika typer av modeller + en baslinjemetod

Först beskriver ni era metoder/modeller teoretiskt i generella termer. (sektionen ovan)

Sen ska ni beskriva era "praktiska metod": hur ni applicerar era metoder/modeller på ert specifika problem. Ni ska alltså har ett strukturerat arbetssätt och ni ska beskriva det övergripande här. Sen under Resultat visar ni resultatet av er applicerade metod. 

 - När man analyserar data är det naturligt att ha en informell initial fas där men explorativt utforskar data och olika metoder kopplat till problemet. Denna fas behöver man oftast inte skriva om under Metod. Notera: denna fas ska vara relativt kort i tid.
 - Sen när ni fått ungefärlig bild av vad som ni behöver göras för att analysera er frågeställning så ska ni bestämma ett **strukturerat upplägg** på hur ni ska använda era metoder/modeller på ert problem.
    ...
    - Vilka modellval/inställningar/hyperparametrar ska göras? Detta är ofta en svår och viktig fråga. Förklara hyperparametrarna betyder.
      - Ofta behöver man fixera några inställningar/hyperparametrar (pga man behöver avgränsa och tidsbegränsningar)
      - Andra inställningar/hyperparametrar vill man hitta så optimala värden för som möjligt (ofta med hjälp av korsvalidering/valideringsdata) $\rightarrow$ ni vill göra en **hyperparametersökning** för era olika metoder/modeller
    ...
    - Hur metoder/modellerna ska utvärderas och jämföras. Val av utvärderingsmått.
...
Tänk att andra analytiker ska kunna ta ert "recept" och använda det på er data eller annan liknande data.

Förklara vilka mjukvaror/program/paket som ni använder för att tillämpa er metod. Citera R-paket.
...
Här ska vi se utfallet av er parktiska metod.
...
Det rekommenderas att man gör någon variant av följande upplägg:

- Om man gör hyperparamersökning/modellval inom en metodfamilj: ofta vill man visa en tabell med resultat per familj, där varje rad är en hyperparameterkombination/modellinställning + utvärderingsmått. Presentera vad som är den bästa modellvalet inom varje metodfamilj. Beroende på hur många kombinationer av modeller som används så kan man ha en tabell för alla modeller. Notera: Om man testar många modeller, då brukar det ofta vara rimligt att visa de bästa 5-10 modellerna per metodfamilj. 
- En jämförande tabell där man visar utvärderingsmått för den bästa modellen från varje metodfamilj för träningsdata och valideringsdata.
- Om det är relevant: mer inzoomade analyser per metodfamilj (tex om man vill undersöka viktiga variabler, residualanalys etc)
- Till sist: Om man använder testdata så visar man utvärderingsmått för **en modell** på testdata = den modell som man anser är bäst av alla tidigare modeller som tagits upp.
...
# Diskussion
Här ingår en kritisk granskning och diskussion av resultaten samt en utvärdering av lämpligheten av de använda metoderna. Här görs även kopplingar tillbaka till tidigare studier som gåtts igenom i bakgrunden med referenser i texten.
...
Om vill kan man ha "Resultat och diskussion" som en gemensam sektion.
...
Återkoppling till planeringen i Datainlämningen:

Reflektion kring generativ AI och lärandemålen i kursen:
