...
The original HIGGS dataset contains approximately 11 million observations. Each observation consists of a binary response variable and 28 explanatory variables. Since the full dataset is very large, we use a smaller subset prepared in advance for this tutorial.

The validation data contain the first 1,000 observations from the original dataset, and the training data contain the next 10,000 observations. This gives the same training and validation observations as in the original TensorFlow tutorial.

Specify the paths to the prepared data files and read them into R:

```{r}
FEATURES <- 28

path_validation <- "HIGGS_validation.csv"
path_training <- "HIGGS_training.csv"
path_validation <- "/home/joswi05/Documents/732G57_admin/2026/datasets_big/HIGGS_validation.csv"
path_training <- "/home/joswi05/Documents/732G57_admin/2026/datasets_big/HIGGS_training.csv"

validation_data <- read.csv(path_validation)
training_data <- read.csv(path_training)
```

Prepare the response variable and explanatory variables:

```{r}
x_val <- as.matrix(validation_data[, -1])
y_val <- as.matrix(validation_data[, 1, drop = FALSE])

x_train <- as.matrix(training_data[, -1])
y_train <- as.matrix(training_data[, 1, drop = FALSE])
```
...
The training data contain 10,000 observations with 28 explanatory variables, while the validation data contain 1,000 observations.
...
Use `keras3::learning_rate_schedule_*()` to reduce the learning rate over time:
...
compile_and_fit <- function(model, name, optimizer = NULL, max_epochs = 10000) {
  ...
  model %>% compile(
    ...
    metrics = list(
      metric_binary_crossentropy(name = "binary_crossentropy"),
      "accuracy"
    )
  )
...
```{r}
tiny_model <- keras_model_sequential(input_shape = c(FEATURES)) %>%
...
```{r}
small_model <- keras_model_sequential(input_shape = c(FEATURES)) %>%
  # `input_shape` is specified so that `summary()` works before training.
  layer_dense(16, activation = 'elu') %>%
  layer_dense(16, activation = 'elu') %>%
  layer_dense(1)
```
...
```{r}
medium_model <- keras_model_sequential(input_shape = c(FEATURES)) %>%
...
```{r}
large_model <- keras_model_sequential(input_shape = c(FEATURES)) %>%
...
plot_histories(size_histories,smooth = TRUE)
plot_histories(size_histories,smooth = FALSE)
```

Note: All the above training runs used the `callback_early_stopping()` to end the training once it was clear the model was not making progress.
...
```{r}
l2_model <- keras_model_sequential(input_shape = c(FEATURES)) %>%
  layer_dense(512, activation = 'elu',
...
plot_histories(regularizer_histories,smooth = TRUE)
```
...
```{r, eval = FALSE}
# Use one observation with 28 explanatory variables
features <- x_train[1, , drop = FALSE]

# Run the observation through the model
result <- l2_model(features)

# Extract and sum the regularization losses
...
```{r}
dropout_model <- keras_model_sequential(input_shape = c(FEATURES)) %>%
  layer_dense(512, activation = 'elu') %>%
  layer_dropout(0.5) %>%
  layer_dense(512, activation = 'elu') %>%
  layer_dropout(0.5) %>%
...
```{r}
combined_model <- keras_model_sequential(input_shape = c(FEATURES)) %>%
