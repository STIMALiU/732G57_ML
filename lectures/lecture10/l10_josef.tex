...
\begin{frame}{K-medoid klustring - PAM}
...
\begin{frame}{PAM-algoritmen: BUILD och SWAP}
    PAM söker $K$ medoider som minimerar summan av avstånden
    till observationernas närmaste medoid.

    \begin{enumerate}
        \item \textbf{BUILD:} Välj stegvis $K$ observationer som
              initiala medoider.

        \item Skapa kluster genom att tilldela varje observation till sin närmaste medoid.

        \item \textbf{SWAP:} Undersök om en medoid kan bytas mot
              en observation som inte är medoid.

        \item Genomför det byte som minskar den totala kostnaden mest.

        \item Upprepa tills inget byte längre förbättrar lösningen.
    \end{enumerate}

    \begin{greenbox}
        Medoiderna är alltid faktiska observationer i datamängden.
    \end{greenbox}
\end{frame}

\begin{frame}{PAM: BUILD-fasen}
    Målet är att välja $K$ bra startmedoider.

    \begin{enumerate}
        \item Välj först den observation som ger lägst total kostnad
              om den används som enda medoid.

        \item Välj därefter den observation som ger den största
              minskningen av den totala kostnaden. Spara som nästa medoid.

        \item Upprepa tills $K$ medoider har valts.
    \end{enumerate}

    \vspace{0.2cm}

    \begin{itemize}
        \item BUILD är en girig (greedy) algoritm.
        \item Försöker sprida ut medoiderna över datamängden.
        \item Ger ofta betydligt bättre startvärden än ett rent slumpmässigt val.
    \end{itemize}
\end{frame}

\begin{frame}{PAM: SWAP-fasen}
    Efter BUILD-fasen försöker PAM förbättra lösningen.

    \begin{enumerate}
        \item Välj en aktuell medoid.

        \item Välj en observation som inte är medoid.

        \item Beräkna hur den totala kostnaden påverkas om
              de två observationerna byter plats.

        \item Genomför det byte som ger störst förbättring.

        \item Upprepa tills inget byte längre minskar kostnaden.
    \end{enumerate}

    \vspace{0.2cm}

    \begin{greenbox}
        SWAP-fasen spelar en liknande roll som centroiduppdateringen
        i K-means.
    \end{greenbox}
\end{frame}

\begin{frame}{PAM-algoritmen - detaljer}
    ...
    söker PAM efter $K$ medoider: $c_1,\ldots,c_K$ som minimerar
...
\begin{enumerate}
    \item $c_1,\ldots,c_K \leftarrow \texttt{BUILD}(X)$

    \item Tilldela varje observation till närmaste medoid.

    \item while(en förbättrande swap existerar)
    \begin{itemize}
        \item $\texttt{SWAP}(c_1,\ldots,c_K)$
        \item Tilldela observationerna till närmaste medoid.
    \end{itemize}

    \item returnera $c_1,\ldots,c_K$
\end{enumerate}
\end{frame}

\begin{frame}{PAM: SWAP-fasen - detaljer}

    Låt
    \[
        M=\{c_1,\ldots,c_K\}
    \]
    vara de aktuella medoiderna.

    \begin{itemize}
        \item Undersök alla möjliga byten
        \[
            c \in M,
            \qquad
            x \notin M.
        ...
        \item Antal kandidatswaps: $K(n-K).$

        \item För varje swap beräknas den nya kostnaden $L$.

        \item Genomför den swap som minskar $L$ mest.

        \item Upprepa tills ingen förbättring är möjlig.
    \end{itemize}
    ...
    \[
        \text{PAM: ungefär } \mathcal{O}(Kn^2)
        \qquad
        \text{K-means: } \mathcal{O}(Kn)
    \]

\end{frame}

\begin{frame}{PAM med slumpmässiga startvärden}
    BUILD-fasen kan ersättas med slumpmässiga startvärden:

    \begin{enumerate}
        \item Välj $K$ observationer slumpmässigt som startmedoider.

        \item Tilldela varje observation till sin närmaste medoid.

        \item Använd SWAP-fasen för att förbättra lösningen.

        \item Upprepa med flera slumpmässiga startvärden.

        \item Behåll den lösning som ger lägst total kostnad.
    \end{enumerate}

    \vspace{0.2cm}

    \begin{itemize}
        \item Liknar användningen av \texttt{nstart} i K-means.
        \item I \texttt{cluster::pam()} används
              \texttt{medoids = "random"} och \texttt{nstart}.
    \end{itemize}
\end{frame}

\begin{frame}{CLARA (Clustering Large Applications)}

    CLARA är en skalbar variant av PAM för stora datamängder.

    \begin{enumerate}
        \item Dra ett slumpmässigt stickprov av observationer.

        \item Kör PAM på stickprovet.

        \item Använd de erhållna medoiderna för att klustra
              hela datamängden.

        \item Upprepa för flera stickprov.
    ...
    \begin{itemize}
        \item Snabbare än PAM för stora datamängder.
        \item Kan använda samma avståndsmått som PAM.
        \item Resultatet beror på vilka stickprov som dras.
    \end{itemize}

\end{frame}

\begin{frame}{Avståndsmått för PAM}

    PAM kan använda många olika avståndsmått eftersom algoritmen
    endast kräver ett avstånd $d(\cdot,\cdot)$. Exempel

    \begin{itemize}

        \item \textbf{Cosinusavstånd}
        \item \textbf{Jaccard-avstånd}
        \item \textbf{Gower-avstånd}:
        Kan hantera numeriska, kategoriska,
        ordinala och binära variabler samtidigt.
    \end{itemize}

\end{frame}

\begin{frame}{Cosinusavstånd}

    Cosinusavstånd mäter skillnaden i \textbf{riktning}
    mellan två vektorer.
    ...
    \begin{itemize}
        \item Bygger på vinkeln mellan två vektorer.
        \item Oberoende av vektorernas längd.
        \item Två vektorer med samma riktning får avstånd 0.
        \item Två ortogonala vektorer får avstånd 1 ( = mest olika)
    \end{itemize}
    ...
    \begin{itemize}
        \item Textdata
        \item Dokumentklustring.
        \item Embeddings från språk- och bildmodeller.
    \end{itemize}

\end{frame}

\begin{frame}{Jaccard-avstånd}

    Jaccard-avstånd används ofta för asymmetriska binära data.
...
\begin{itemize}
    \item $a$ = antal variabler där båda observationerna har värdet 1,
    \item $b$ = antal variabler där den första observationen har värdet 1 och den andra 0,
    \item $c$ = antal variabler där den första observationen har värdet 0 och den andra 1.
\end{itemize}

    Egenskaper:

    \begin{itemize}
        \item Gemensamma nollor påverkar inte avståndet. Fokuserar på närvaro snarare än frånvaro.
    \end{itemize}

    Vanliga tillämpningar: Marknadskorgsanalys, dokument representerade med ordindikatorer. Frånvaro/närvaro-data


\end{frame}

\begin{frame}{Gower-avstånd}

    Gower-avstånd kan hantera blandade datatyper.
    ...
    där $d_{ijk}$ är avståndet för variabel $k$.
    ...
    \begin{itemize}
        \item Numeriska variabler skalas till intervallet $[0,1]$.
        \item Kategoriska variabler behandlas via match/mismatch.
        \item Kan hantera ordinala och binära variabler.
        \item Alla delavstånd kombineras till ett totalavstånd.
    \end{itemize}
    ...
    \begin{itemize}
        \item Kundsegmentering.
        \item Enkätdata.
        \item Data med både numeriska och kategoriska variabler.
    \end{itemize}
...
\begin{frame}{Utvärdera klusteranalys}
    ...
    \begin{itemize}
        \item Finns det kluster i data? Eller har observationerna bara slumpmässiga värden?
        \begin{itemize}
        ...
        Hjälper resutlatet från klustringen oss att lösa problemet?
    \end{itemize}
